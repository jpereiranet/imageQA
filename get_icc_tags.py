...
class GetICCtags(object):
    ...
    def RDcurv(self, s):
        ...
        try:
            ...
            #algunos perfiles necesitan 2 bits más de buffer
            table = struct.unpack('>%dH' % count, s[14:])

        if count == 1:
            s =  dict(gamma=table[0] * 2 ** -8)
            if s['gamma'] == 0.0:
               #algunos perfiles arrojan una gamma=0.0 y da error
                s = dict(gamma=self.default_gamma)
            return s

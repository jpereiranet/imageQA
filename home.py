    ...
    def open_multiple_test(self):
        ...
        try:
            ...
            if is_roi:
                ...
                if hasattr(self.ui, 'coordenadasReales'):
                    img_coordinates = self.ui.coordenadasReales
                else:
                    return AppWarningsClass.critical_warn("No region coordinates!")
            ...
            charts = ChartPatternsClass()
            switch = charts.get_switch(self.ui.chartIndex)
            if switch:
                dialog.ui.setupUi(dialog)
                dialog.setAttribute(Qt.WA_DeleteOnClose)
                dialog.exec_()
            else:
                return AppWarningsClass.critical_warn("You must select a chart type first!")
        ...
        try:
            ...
            if self.ui.imMode == "RGB":
                ...
                de_values = [
                    ...
                    ("L-OECF", oecf_values.get_oecf_values("L-OECF")),
...
if __name__ == '__main__':
    ...
    #root = QtGui.QGuiApplication(sys.argv)
    ...
    pixmap = QPixmap(DefinePathsClass.create_resource_path('splash.jpg'), 'JPEG')
    ...
    Bar = QProgressBar(splash_screen)
    Bar.setMaximum(10)
    ...
    for i in range(1, 3):
        ...
        if i > 4 and texto[i] in texto:
            splash_screen.showMessage(texto[i], Qt.AlignTop, Qt.black)

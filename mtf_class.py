...
class GetMTFClass:
    ...
    def compute_esf(self):
        ...
        for i in range(0, row):
            ...
            dimension_a = len(temp_v)
            dimension_b = len(array_values_near_edge[i, :])

            if dimension_a != dimension_b :
                return AppWarningsClass.critical_warn("Edge error detect! perhaps not a slanted edge, check the slanted angle")
